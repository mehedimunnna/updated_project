...
int main() {
    ...
    for (i = 0; i < m; i++) {
        ...
        fwrite(&pl1[i], sizeof(struct batsman), 1, fbatsman);
    }
    fclose(fbatsman);
    ...
    for (i = 0; i < n; i++) {
        ...
        fwrite(&pl2[i], sizeof(struct bowler), 1, fbowler);
    }
    fclose(fbowler);
    ...
    do {
        ...
        switch (choice) {
            case 1:
                ...
                fbatsman = fopen("batsman.txt", "r");
                fseek(fbatsman, plno * sizeof(struct batsman), SEEK_SET);
                fread(&pl1[plno], sizeof(struct batsman), 1, fbatsman);
                fclose(fbatsman);
            ...
            case 2:
                ...
                fbowler = fopen("bowler.txt", "r");
                fseek(fbowler, plno * sizeof(struct bowler), SEEK_SET);
                fread(&pl2[plno], sizeof(struct bowler), 1, fbowler);
                fclose(fbowler);
                ...
                pl2[plno].econ = pl2[plno].runsgv / (float)pl2[plno].overs;
            ...
            case 3:
                ...
                pl3.toruns = 0;
                for (i = 0; i < m; i++) {
                    fread(&pl1[i], sizeof(struct batsman), 1, fbatsman);
                ...
                fbowler = fopen("bowler.txt", "r");
                for (i = 0; i < n; i++) {
                    fread(&pl2[i], sizeof(struct bowler), 1, fbowler);
                    pl2[i].econ = pl2[i].runsgv / (float)pl2[i].overs;
